...
ml_weights = np.loadtxt('mlo_weights.txt')
ml_weights = torch.from_numpy(ml_weights)

mlo = torch.mm(features, ml_weights.float())

np.savetxt('gcn_mlo_citeseer.txt',mlo.numpy(),fmt='%1.3f')
...
#torch.save(model.state_dict(), "./model/{}/model_weights.pth".format(args.dataset))
...
ml_weights = np.loadtxt('mlo_weights_cora.txt')
ml_weights = torch.from_numpy(ml_weights)
